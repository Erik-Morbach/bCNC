...
class Sender:
    ...
    def __init__(self):
        ...
        # Display alarm message if true
        self._alarm = ThreadVar.ThreadVar(True)
    ...
    def _saveConfigFile(self, filename=None):
        ...
        Utils.setUtf("File", "dir",   os.path.dirname(
        ...
        if ext == ".probe":
            ...
            try:
                ...
                tkMessageBox.showinfo(
                    "Open 3D Mesh", "Importing of 3D mesh files in .STL and .PLY format is supported by SliceMesh plugin.\nYou can find it in CAM->SliceMesh.")
    ...
    def _wcsSet(self, x, y, z, a=None, b=None, c=None, wcsIndex=None):		self.mcontrol._wcsSet(
        x, y, z, wcsIndex=wcsIndex)  # FIXME Duplicate with ControlPage
        ...
        try:
            ...
            with open("myLog.txt", 'a') as logfile:
                logfile.write("EXCEPTION {} {} : {}".format(
                    time.ctime(), "ReadExecutor", str(traceback.format_exc())))
            traceback.print_exc()
        ...
        try:
            while self.readThread:
                ...
                try:
                    ...
                    if CNC.vars['debug']: print(line, end='')
                except:
                    self.log.put((Sender.MSG_RECEIVE, str(sys.exc_info()[1])))
                    continue
                buff += line
            ...
            with open("myLog.txt", 'a') as logfile:
                logfile.write("EXCEPTION {} {} : {}".format(
                    time.ctime(), "Read", str(traceback.format_exc())))
            traceback.print_exc()
        ...
        try:
            ...
            with open("myLog.txt", 'a') as logfile:
                logfile.write("EXCEPTION {} {} :\n{}".format(
                    time.ctime(), "WriteRt", traceback.format_exc()))
            traceback.print_exc()
    ...
    def shouldSend(self):
        # is in wait
        if not self.ioData.sio_wait and not self._pause.value:
            return True
        # do not have commands to send, then simply continue
        if self.ioData.haveCommand() and self.hasNewCommand():
            return True
        return False
    ...
    def executeInternalCommand(self, cmd):
        code = cmd.src
        if isinstance(code, tuple):
            return self.executeTupleInternalCommand(code)
    ...
    def appendCodeToDeque(self, cmds):
        ...
        if len(cmds) == 0:
            ...
            return
        # consider that is already added to _runLines
        self._runLines.assign(lambda rl: rl + len(cmds) - 1)
    ...
    def serialIOWrite(self):
        ...
        try:
            ...
            with open("myLog.txt", 'a') as logfile:
                logfile.write("EXCEPTION {} {} : {}".format(
                    time.ctime(), "Write", str(traceback.format_exc())))
            traceback.print_exc()
